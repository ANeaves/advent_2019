...
class IntcodeComputer():
    ...
    def __init__(self, code=[], prog_input=0, name=None):
        self.code = code
    ...
    def set_input(self, prog_input):
        logging.info("SETTING INPUT TO %s", prog_input)
        self.input = prog_input
    ...
    def put(self, put_addr, param_modes):
        ...
        if isinstance(self.input, list):  # hacky way to take multiple inputs
            self.memory[put_addr] = self.input[self.input_count]
            self.input_count += 1
        else:
            self.memory[put_addr] = self.input
        logging.debug("INPUTTING: %d", self.memory[put_addr])
